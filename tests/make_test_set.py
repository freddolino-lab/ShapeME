...
for name, param in zip(cats.names, cats):
    param.add_shape_param(
        dsp.ShapeParamSeq(
            name="MGW",
            params=indata.pull_entry(name).seq,
        )
    )

cats.normalize_params()
center, spread = cats.center_spread["MGW"]
correct_motif = dsp.ShapeParamSeq(
    name="MGW",
    params = list(5.0*np.ones(15)),
)
#correct_motif.normalize_values(center, spread)
for i, param in enumerate(cats):
    if cats.values[i] == 1 or i == 10:
        param.data["MGW"].params[300:315] = (
            correct_motif.params
            * np.random.normal(1.0, spread, 15)
        )
    else:
